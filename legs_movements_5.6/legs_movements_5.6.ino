...
//HardwareSerial mySerial(0); //串口0，RX:3, TX:1 ,用于ibus
// HardwareSerial Serial1(1);//串口1，RX:9, TX:10 ,用于舵机控制
HardwareSerial mySerial2(2);//串口2，RX:16, TX:17 ,接收openmv数据
...
void move_servo(int servo_1, int servo_2, int x, int y, int Time){ //servo_1 is the showder servo ID; servo_2 is the elbow servo ID
  ...
  //
  LobotSerialServoMove(Serial1, servo_1, map(shouderAngle,60,300,0,1000), Time);
  LobotSerialServoMove(Serial1, servo_2, map(elbowAngle,60,300,0,1000), Time);
}
...
//double_leg
void Double_leg_control(int Push_down) {
  //点位宏定义 ; 这个点位可能需要做实验
  #define Y1 175  - Push_down//170 如果在压低地盘
  #define Y2 180  - Push_down
  #define X1 -10
  #define X2 0
  #define X3 10
  look() ;//判断地形
...
void Single_leg_control(){
  ...
  LobotSerialServoMove(Serial1, ID1, 312, 500);
  LobotSerialServoMove(Serial1, ID7, 688, 500) ;

  LobotSerialServoMove(Serial1, ID4, 312, 500) ;
  LobotSerialServoMove(Serial1, ID10, 688, 500) ;
  //
...
void setup(){
  ...
  Serial.begin(115200,SERIAL_8N1, 3, 1); //ibus
...
void loop(){
  ...
  */
  ...
  //Single_leg_control() ;
  //Double_leg_control(0) ;
  //Double_leg_slope_control() ;
  

  
  if (Flag_down) Double_leg_control(25) ;
  else Double_leg_control(0) ;
